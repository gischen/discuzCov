...
class Core_Image
{
	...
	public function resize()
	{
		...
		{
			die('Core_Image: Source file not exists: ' . $this->source_image);
		}
		...
		{
			default:
				//die('Core_Image: Can\'t detect output image\'s type: ' . $this->source_image);
				return false;
			break;
	...
	private function imageProcessGD()
	{
		...
		{
			die('Core_Image: Function not exists for output: ' . $func_output);
		}
		...
		{
			die('Core_Image: Function not exists for output: ' . $func_create);
		}
		...
		else if ($this->option == IMAGE_CORE_OP_OUTPUT)
		{
			...
			{
				die('Core_Image: HTTP already sent, can\'t output image to browser.');
			}
